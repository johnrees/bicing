class StationsController < ApplicationController
  ...
  def map
    ...
    @stations = Station.eager_load(:readings).where("readings.recorded_at <= ?", (params[:datetime] ? (params[:datetime].to_datetime - 1.hour) : Time.now)).group('readings.id','stations.id').order("readings.recorded_at DESC")
